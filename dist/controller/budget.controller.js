...
function toDecimalInput(amount) {
...
class BudgetController {
    async adminUploadSpendDocument(req, res) {
        try {
            const file = req.file;
            if (!file) {
    ...
    async upsertPolicy(req, res) {
        try {
            ...
            const policy = await prisma_1.prisma.budgetPolicy.upsert({
    ...
    async listPolicies(req, res) {
        try {
            ...
            const data = await prisma_1.prisma.budgetPolicy.findMany({
                where: { year },
    ...
    async upsertRawatJalanPolicy(req, res) {
        try {
            const body = req.body;
            const year = body.year ?? currentYear();
            const position = body.position;
            const annualAmount = body.annualAmount;
            if (!position)
                return res.status(400).json({ success: false, message: 'position wajib dikirim' });
            if (annualAmount === undefined || annualAmount === null || annualAmount === '') {
                return res.status(400).json({ success: false, message: 'annualAmount wajib dikirim' });
            }
            const policy = await prisma_1.prisma.rawatJalanPolicy.upsert({
                where: { year_position: { year, position } },
                update: { annualAmount: toDecimalInput(annualAmount) },
                create: { year, position, annualAmount: toDecimalInput(annualAmount) },
                select: { id: true, year: true, position: true, annualAmount: true },
    ...
    async listRawatJalanPolicies(req, res) {
        try {
            const year = req.query.year ? Number(req.query.year) : currentYear();
            const data = await prisma_1.prisma.rawatJalanPolicy.findMany({
                where: { year },
                orderBy: [{ position: 'asc' }],
                select: { id: true, year: true, position: true, annualAmount: true },
            });
    ...
    async upsertMelahirkanPolicy(req, res) {
        try {
            const body = req.body;
            const year = body.year ?? currentYear();
            const position = body.position;
            ...
            const policy = await prisma_1.prisma.melahirkanPolicy.upsert({
                where: { year_position: { year, position } },
                update: { annualAmount: toDecimalInput(annualAmount) },
                create: { year, position, annualAmount: toDecimalInput(annualAmount) },
                select: { id: true, year: true, position: true, annualAmount: true },
    ...
    async listMelahirkanPolicies(req, res) {
        try {
            const year = req.query.year ? Number(req.query.year) : currentYear();
            const data = await prisma_1.prisma.melahirkanPolicy.findMany({
                where: { year },
                orderBy: [{ position: 'asc' }],
                select: { id: true, year: true, position: true, annualAmount: true },
            });
    ...
    async upsertRawatInapPolicy(req, res) {
        try {
            const body = req.body;
            const year = body.year ?? currentYear();
            const position = body.position;
            const serviceType = normalizeEnum(body.serviceType);
            const capAmount = body.capAmount;
            if (!position)
                return res.status(400).json({ success: false, message: 'position wajib dikirim' });
            if (!serviceType)
                return res.status(400).json({ success: false, message: 'serviceType wajib dikirim' });
            if (capAmount === undefined || capAmount === null || capAmount === '') {
                return res.status(400).json({ success: false, message: 'capAmount wajib dikirim' });
            }
            const allowed = ['TARIF_KAMAR_DAYS', 'TANPA_OPERASI', 'OPERASI'];
            if (!allowed.includes(serviceType)) {
                return res.status(400).json({ success: false, message: 'serviceType tidak valid' });
            }
            const policy = await prisma_1.prisma.rawatInapPolicy.upsert({
    ...
    async listRawatInapPolicies(req, res) {
        try {
            const year = req.query.year ? Number(req.query.year) : currentYear();
            const data = await prisma_1.prisma.rawatInapPolicy.findMany({
                where: { year },
    ...
    async listRawatJalanMedicals(req, res) {
        try {
            const onlyActive = req.query.active === undefined ? true : String(req.query.active) !== 'false';
            const data = await prisma_1.prisma.rawatJalanMedical.findMany({
    ...
    async createRawatJalanMedical(req, res) {
        try {
            const body = req.body;
            const name = (body.name ?? '').trim();
            if (!name)
                return res.status(400).json({ success: false, message: 'name wajib dikirim' });
            const created = await prisma_1.prisma.rawatJalanMedical.create({
    ...
    async setRawatJalanMedicalActive(req, res) {
        try {
            const id = typeof req.params.id === 'string' ? req.params.id : '';
            if (!id)
                return res.status(400).json({ success: false, message: 'id wajib dikirim' });
            const body = req.body;
            if (typeof body.isActive !== 'boolean') {
                return res.status(400).json({ success: false, message: 'isActive wajib boolean' });
            }
            const updated = await prisma_1.prisma.rawatJalanMedical.update({
                where: { id },
    ...
    async listRawatInapEpisodeOptions(req, res) {
        try {
            const isAdmin = req.user?.role === 'ADMIN';
            const onlyActive = !isAdmin ? true : String(req.query.active ?? 'true') !== 'false';
            const data = await prisma_1.prisma.rawatInapEpisodeOption.findMany({
                where: onlyActive ? { isActive: true } : undefined,
                orderBy: [{ name: 'asc' }],
                select: { id: true, name: true, isActive: true, createdAt: true, updatedAt: true },
            });
    ...
    async createRawatInapEpisodeOption(req, res) {
        try {
            const body = req.body;
            const name = (body.name ?? '').trim();
            if (!name)
                return res.status(400).json({ success: false, message: 'name wajib dikirim' });
            const created = await prisma_1.prisma.rawatInapEpisodeOption.create({
                data: { name, isActive: true },
                select: { id: true, name: true, isActive: true, createdAt: true, updatedAt: true },
            });
            return res.status(201).json({
    ...
    async setRawatInapEpisodeOptionActive(req, res) {
        try {
            const id = typeof req.params.id === 'string' ? req.params.id : '';
            if (!id)
                return res.status(400).json({ success: false, message: 'id wajib dikirim' });
            ...
            const updated = await prisma_1.prisma.rawatInapEpisodeOption.update({
                where: { id },
                data: { isActive: body.isActive },
                select: { id: true, name: true, isActive: true, createdAt: true, updatedAt: true },
            });
    ...
    async adminCreateRawatInapEpisode(req, res) {
        try {
            const body = req.body;
            ...
            const result = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                const serviceTypes = ['TARIF_KAMAR_DAYS', 'TANPA_OPERASI', 'OPERASI'];
                const policies = await tx.rawatInapPolicy.findMany({
                    where: { year, position: user.position, serviceType: { in: serviceTypes } },
                    select: { serviceType: true, capAmount: true },
                });
                const capMap = new Map();
                for (const p of policies)
                    capMap.set(p.serviceType, p.capAmount);
            ...
            const message = e instanceof Error ? e.message : 'Unknown error';
            const isClient = e instanceof Error &&
            ...
            return res.status(isClient ? 400 : 500).json({
                success: false,
                message: isClient ? e.message : 'Internal server error',
                error: process.env.NODE_ENV === 'development' ? message : undefined,
            });
        }
    }
    async adminListRawatInapEpisodes(req, res) {
        try {
            const userId = typeof req.query.userId === 'string' ? req.query.userId : '';
            if (!userId)
                return res.status(400).json({ success: false, message: 'userId wajib dikirim' });
            const year = req.query.year ? Number(req.query.year) : currentYear();
            const data = await prisma_1.prisma.rawatInapEpisode.findMany({
                where: { userId, year },
    ...
    async getMyBudget(req, res) {
        try {
            ...
            const { rawatJalan, melahirkan, rawatInap } = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                const allocatedSum = agg._sum.allocated ?? new client_1.Prisma.Decimal(0);
                const spentSum = agg._sum.spent ?? new client_1.Prisma.Decimal(0);
                return {
                    rawatJalan: rj,
                    melahirkan: mel,
                ...
                data: {
                    ...
                    melahirkan: melahirkan
                        ? {
                            ...
                            remaining: melahirkanRemaining.toString(),
                        }
                        : null,
                },
            });
            ...
            const message = e instanceof Error ? e.message : 'Unknown error';
            const isPolicyMissing = e instanceof Error && e.message.toLowerCase().includes('policy');
    ...
    async adminListUserBudgets(req, res) {
        try {
            const year = req.query.year ? Number(req.query.year) : currentYear();
            const result = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                for (const u of users) {
                    ...
                    if (!rj) {
                        const policy = await tx.rawatJalanPolicy.findUnique({
                            where: { year_position: { year, position: u.position } },
                        });
                        if (!policy) {
                    ...
                    const riAllocated = agg._sum.allocated ?? new client_1.Prisma.Decimal(0);
                    const riSpent = agg._sum.spent ?? new client_1.Prisma.Decimal(0);
                    ...
                    out.push({
                        ...
                        melahirkan: mel
                            ? {
                                ...
                                remaining: melRemaining.toString(),
                            }
                            : null,
                    });
                }
    ...
    async adminSpendForUser(req, res) {
        try {
            const body = req.body;
            if (!body.userId) {
                return res.status(400).json({ success: false, message: 'userId wajib dikirim' });
            }
            const benefitTypeNormalized = normalizeEnum(body.benefitType);
            const useRiDetail = benefitTypeNormalized === 'RAWAT_INAP' &&
            ...
            if (!useRiDetail && !isRawatInap) {
                ...
                const amountNumber = typeof body.amount === 'number'
                    ? body.amount
            ...
            const documentPublicId = typeof body.documentPublicId === 'string' ? body.documentPublicId.trim() : '';
            const documentOriginalName = typeof body.documentOriginalName === 'string' ? body.documentOriginalName.trim() : '';
            const spendDocument = documentUrl || documentPublicId || documentOriginalName
                ? {
            ...
            const result = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (benefitTypeNormalized) {
                    if (benefitTypeNormalized === 'RAWAT_JALAN') {
                        ...
                        if (!medicalId)
                            throw new Error('rawatJalanMedicalId wajib dikirim untuk RAWAT_JALAN');
                        const medical = await tx.rawatJalanMedical.findUnique({
                            where: { id: medicalId },
                            select: { id: true, isActive: true },
                        });
                        if (!medical || medical.isActive !== true)
                            throw new Error('Rawat Jalan medical tidak valid/aktif');
                        ...
                        if (newSpent.gt(balance.allocated)) {
                            return { ok: false, status: 409, message: 'Insufficient remaining budget' };
                        }
                    ...
                    if (benefitTypeNormalized === 'MELAHIRKAN') {
                        if (user.gender !== 'FEMALE')
                            throw new Error('Melahirkan hanya untuk user FEMALE');
                    ...
                    if (benefitTypeNormalized === 'RAWAT_INAP') {
                        ...
                        if (useRiDetail) {
                            ...
                            const roomAmount = new client_1.Prisma.Decimal(roomRatePerDay).mul(days);
                            const procedureAmt = new client_1.Prisma.Decimal(procedureAmountNum);
                            const procedureType = (isOperasi ? 'OPERASI' : 'TANPA_OPERASI');
                            ...
                            const balanceProc = await tx.rawatInapEpisodeBalance.findUnique({
                                where: { episodeId_serviceType: { episodeId, serviceType: procedureType } },
                            });
                            if (!balanceRoom || !balanceProc)
                                throw new Error('Balance episode tidak ditemukan');
                            if (roomAmount.gt(0)) {
                                // Plafon kamar berlaku per-hari (bukan akumulasi total episode).
                                const roomRateCap = balanceRoom.allocated;
                                const roomRatePerDayDec = new client_1.Prisma.Decimal(roomRatePerDay);
                            ...
                            if (procedureAmt.gt(0)) {
                                ...
                                await tx.budgetLedger.create({
                                    data: {
                                        ...
                                        benefitType: 'RAWAT_INAP',
                                        rawatInapEpisodeId: episodeId,
                                        rawatInapServiceType: procedureType,
                                        ...spendDocument,
                                        note: (body.note ?? 'Rawat Inap') +
                        ...
                        const serviceType = normalizeEnum(body.rawatInapServiceType);
                        const allowed = ['TARIF_KAMAR_DAYS', 'TANPA_OPERASI', 'OPERASI'];
                        ...
                        const balance = await tx.rawatInapEpisodeBalance.findUnique({
                            where: { episodeId_serviceType: { episodeId, serviceType: serviceType } },
                        });
                        if (!balance)
                            throw new Error('Balance episode tidak ditemukan');
                        ...
                        await tx.budgetLedger.create({
                            data: {
                                ...
                                benefitType: 'RAWAT_INAP',
                                rawatInapEpisodeId: episodeId,
                                rawatInapServiceType: serviceType,
                                ...spendDocument,
                ...
                if (!balance) {
                    ...
                    if (!policy) {
                        throw new Error(`Budget policy belum diset untuk year=${year} position=${user.position}`);
                    }
                ...
                const spendCategoryNormalized = spendCategoryRaw === undefined || spendCategoryRaw === null || spendCategoryRaw === ''
                    ? null
    ...
    async adminResetUserTransactions(req, res) {
        try {
            const body = req.body;
            const userId = (body.userId ?? '').trim();
            if (!userId) {
    ...
    async adminListOptionTrends(req, res) {
        try {
            const year = req.query.year ? Number(req.query.year) : currentYear();
            const [rjRows, riRows] = await Promise.all([
                prisma_1.prisma.budgetLedger.findMany({
                    where: {
                        year,
                        type: 'SPEND',
                        benefitType: 'RAWAT_JALAN',
    ...
    async adminListUserLedger(req, res) {
        try {
            const userId = typeof req.query.userId === 'string' ? req.query.userId : '';
            if (!userId) {
                return res
            ...
            const entries = await prisma_1.prisma.budgetLedger.findMany({
                where: { userId, year },
